...
> Планируем блюда на неделю → корзина собирается сама → покупаем вместе в реальном времени → купленное оседает в кладовой → готовим по своим рецептам в едином формате → повторяем.
...
3. **Realtime по умолчанию.** Отметил — у партнёра обновилось мгновенно, без обновления страницы.
...
- Общий realtime-список домохозяйства. Видно, **кто добавил** и **кто купил/заказал**.
...
flowchart LR
    ...
    subgraph Railway["Railway ($5–10/мес)"]
        APP[Next.js App Router\n+ tRPC API + SSE]
        DB[(PostgreSQL)]
        APP -- LISTEN/NOTIFY --> DB
    end
    UI -- tRPC (HTTP) --> APP
    APP -- SSE-подписка на household --> UI
...
| Слой | Выбор | Зачем |
|---|---|---|
| Хостинг | **Railway** — и приложение, и Postgres | Один хостинг; always-on Node-процесс ⇒ SSE живёт без ограничений (serverless-режим Railway несовместим с SSE и постоянным коннектом к БД). Честный бюджет с RAM-биллингом: **$5–10/мес**, следить по метрикам в дашборде |
...
| Realtime | **SSE + Postgres LISTEN/NOTIFY** (tRPC subscriptions) | $0, без внешних сервисов; для 2–5 человек — с огромным запасом. Канал на household |
| Клиент-состояние | **TanStack Query** | Оптимистичные мутации, инвалидация по realtime-событиям |
...
### 6.3 Realtime и офлайн

- Подписка `household.<id>` через SSE (tRPC v11 `httpSubscriptionLink` + `splitLink`; серверный `sse.ping`, чтобы прокси не рвали idle-соединения; для будущей RN-фазы — полифилл EventSource). События вида `{entity: "cart"|"pantry"|"menu", op, id}` → клиент инвалидирует запросы и подсвечивает изменение («✓ Куплено: молоко»).
- `LISTEN` живёт на **выделенном Postgres-соединении вне пула Drizzle**, с авто-reconnect и повторным `LISTEN` после обрыва. NOTIFY не персистится, поэтому правило: **SSE-реконнект ⇒ полная инвалидация кэша** запросов household (события за время разрыва потеряны — и это нормально). Для self-hosting: LISTEN/NOTIFY не работает через PgBouncer в transaction mode (на Railway подключение прямое — ок).
...
- Корзина: каталог, автокомплит, отделы + настраиваемый порядок, статусы (нужно/заказано/куплено), «кто берёт», realtime, оптимистичный UI, «завершить закупку», история
