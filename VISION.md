...
3. **Синхронизация без потерь (MVP).** Каждое действие сохраняется сразу; у партнёра изменения подъезжают автоматически при открытии/возврате к экрану, по кнопке «Обновить» (pull-to-refresh) и мягким фоновым поллингом. Мгновенный realtime — осознанно вынесен в пост-MVP (решение 2026-08-19: приложение живёт на Vercel/serverless).
...
flowchart LR
    ...
    subgraph Railway["Railway (~$1–3/мес)"]
        DB[(PostgreSQL)]
    end
...
| Слой | Выбор | Зачем |
|---|---|---|
| Хостинг | **Vercel** (приложение, Hobby) + **Railway** (Postgres) | Vercel — serverless: подходит для tRPC/страниц, но не для долгоживущих соединений ⇒ realtime вынесен в пост-MVP. Бюджет: Vercel $0, Railway Postgres ~$1–3/мес |
