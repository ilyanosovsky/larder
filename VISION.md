...
- Все данные изолированы по household; каждый household-scoped tRPC-запрос (`householdProcedure`) проверяет членство — `publicProcedure`/`protectedProcedure` членство не загружают, это не их зона ответственности. Изоляция — на уровне приложения: каждый statement скоупится по `household_id` (включая write-пути, где id, присланный клиентом, дополнительно проверяется на принадлежность домохозяйству), и тесты `expectScopedByHousehold` принуждают этот паттерн для каждого household-scoped роутера. Составные tenancy-FK в БД (unique `(household_id, id)` + составные внешние ключи) осознанно не вводим — решение 2026-08-21, схема в целом, см. docs/wiki/Development.md; constraints в БД оставлены для конкурентных бизнес-инвариантов (одно домохозяйство на пользователя, одна активная строка корзины на продукт, уникальность `normalizedName`).
