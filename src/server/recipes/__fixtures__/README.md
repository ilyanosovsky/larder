...
"похожие рецепты" carousels are gone. Image *tags* are kept where an
...
Re-capturing one is a deliberate act: these files pin *what the parsers must
survive*, not what a site looks like today, so a test that starts failing is a
