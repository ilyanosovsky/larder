...
**A synchronous ref closes the ± double-tap window.** `onMutate` opens with an awaited `cancelQueries`, so the optimistic patch — and the re-render carrying it — lands a few milliseconds after `mutate()` returns; two taps inside that window would both read the rendered row and both send the same next value. The ref records what each card's last tap asked for, keyed by item id, and is dropped on settle so a failed write falls back to whatever the rolled-back row shows.
