...
**`?cook=1` semantics.** Opening («Готовить», a real `<Link href="…?cook=1">` since this task — gated on the dish actually having at least one step, its own `aria-disabled` case with `dish.cookNoSteps` rather than the generic «скоро») is a normal Next `Link` navigation, which **pushes** a history entry. Closing (✕, or the inline exit confirmation's «Выйти») always calls `router.replace` back to the plain dish URL — never `router.back()` — so the hardware Back gesture (which *pops* the pushed entry) and an explicit close both land on the exact same URL. Back specifically **never** goes through the exit confirmation: `CookingOverlay` only ever reads `searchParams` from the *outside*, in a `useEffect`, and a Back-driven disappearance of `?cook=1` is indistinguishable there from any other close — there is no code path from a `searchParams` change into the confirmation at all. A reload with `?cook=1` still in the address bar reopens on the persisted step (below), because the overlay's "open" state is derived from the URL, not carried in memory.
...
**Only one timer at a time.** Starting a second timer on a different step while one is already running is refused (`aria-disabled` with `cooking.timerBusy`) rather than silently abandoning the first — losing track of a running bake is exactly the failure a kitchen timer exists to prevent.

**Finish signal, iOS-honest (VISION §6.6).** `role="alert"` text inside `cook-timer.tsx`'s own finished state (visible when the step that started the timer is on screen) **and** a permanent, always-mounted `role="status"` live region inside `CookingSession` that fires once regardless of which step is visible when the timer actually rings (`cooking.timerFinishedSr`) — the two exist because the first alone would say nothing if the cook had already navigated away. The audio: a two-chime beep embedded as a WAV data URI (`src/lib/cooking/finish-sound.ts`, ~2.6KB before base64 — small enough to inline rather than fetch, and it has to work offline in this PWA), played once on the «запустить» tap and immediately paused-and-rewound (the standard "prime on a real gesture" trick — iOS only allows an element's *first* `play()` to originate outside a gesture if it was already unlocked by an in-gesture one), then played for real, unprompted, on finish. `navigator.vibrate` is deliberately not called: it is a documented no-op on iOS, and pretending otherwise would be dishonest about what the signal actually is. No Notification API, no Web Push — VISION §6.6 rules both out for iOS PWAs.
...
**Step navigation agrees across buttons, ArrowLeft/ArrowRight, and swipe** — all three call the same pure `stepNavigation(current, total, action)` (`src/lib/cooking/steps.ts`), clamped at both ends. The swipe gesture is a **sibling** of `src/lib/pantry/swipe-commit.ts`'s `decideSwipeCommit`, not a reuse of it: mechanically the same 96px-distance-or-24px/0.5px-per-ms-fling math (`src/lib/cooking/step-swipe.ts`'s `decideStepSwipe`), but `decideSwipeCommit` returns `"have" | "ranOut"` — names that belong to the pantry revision deck's own domain and would misdescribe a step swipe, and generalizing that function into a shared direction-only shape would be a refactor of code this task otherwise never touches. Swipe left → next, swipe right → prev (the same "content advances left" convention every phone carousel uses, and the direction the footer's own «← Назад» / «Далее →» pair implies). Pointer handling copies `revision-mode.tsx`'s discipline: primary-pointer-and-left-button-only, `setPointerCapture`, cancel on `pointercancel`/`lostpointercapture`, drag state cleared *before* the commit decision runs, `prefers-reduced-motion` skips the spring-back animation.

**Manually verified** on a phone-sized viewport against the seeded NYC Cookies (six steps, a 9–11 min bake on one of them): buttons/arrow-keys/swipe all navigate and agree, «шаг N из M» updates and is announced; started the timer, navigated away and back — the clock read a correct, continuously-recomputed value both times, never negative; closed and reopened the tab mid-countdown — resumed counting down from `endsAt`, not reset; let it reach zero — «Готово!» rendered with an audible ping (primed on the earlier «запустить» tap) and the header-level live region also fired while a different step was on screen; exit confirmation appeared past step 1 and while the timer ran, and did not appear on step 1 with no timer or a finished one; the hardware Back gesture closed the overlay silently every time, never showing the confirmation; a reload with `?cook=1` in the address bar reopened on the persisted step. Wake Lock matrix: `navigator.wakeLock` present (desktop Chrome) → status active, no hint rendered; `navigator.wakeLock` deleted via devtools (`delete navigator.wakeLock`) before opening → the honest hint rendered immediately, no crash. (iOS 18.4+ standalone-PWA acquisition itself was not verified on a physical device — out of reach in this environment; the code path is identical to the desktop-Chrome-verified one, gated purely on API presence.)
