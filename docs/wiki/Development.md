...
**A paused mutation is not "busy".** `cart.updateItem`/`setStatus`/`remove` all use the default `networkMode: "online"`, so a write made offline pauses before its request ever leaves (task 2.4) and its `mutateAsync` promise does not settle until the connection returns — sometimes minutes later. `busy` is therefore derived as `isPending && !isPaused` across all three mutations, not from a plain `useState`, and a separate effect releases `busyRef` the moment any of them pauses rather than holding it for the whole round trip. This is safe specifically because these three are last-write-wins, never merging like `cart.add`: a second tap queuing a newer edit behind an already-paused one is an ordinary conflict the queue already resolves. A `queued` boolean (same three `isPaused` flags) renders S3's own «ждёт синхронизации» copy as a status line instead of leaving every control looking permanently stuck.
