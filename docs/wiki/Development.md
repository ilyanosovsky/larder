...
**Applying drops the missing appliances from `recipe.equipment`, and adds none.** A recipe reworked to avoid a mixer no longer needs one, and without the drop S7's banner would keep reporting it missing forever after the fix was applied. Nothing is added: the model is told to use only what the household already has, and inferring a new slug from prose would be a guess about the one field the banner reads.
